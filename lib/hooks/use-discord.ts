...
export function useDiscord() {
  ...
  const fetchChannels = async (serverId: string) => {
    ...
    try {
      ...
      const readableChannels = (data.channels || []).filter((channel: Channel) => channel.type !== "unknown")
  ...
  const fetchNewMessages = async (channelId: string, lastMessageId: string) => {
    ...
    try {
      ...
      setMessages((prevMessages) => {
        ...
        const newMessages = data.messages.filter((msg: Message) => !existingMessageIds.has(msg.id));
        return [...prevMessages, ...newMessages];
      });
    } catch (err) {
      console.error("Error fetching new messages:", err);
      setError(err instanceof Error ? err.message : "Failed to fetch new messages");
    }
  };
